...
export class ChatController {
  ...
  async chatMessages(
    ...
    try {
      // Normalize incoming snake_case fields to camelCase to be tolerant for clients
      // that send `response_mode` or `responseMode`, and `conversation_id` etc.
      // We mutate the body before validation/usage.
      if ((body as any).response_mode && !(body as any).responseMode) {
        (body as any).responseMode = (body as any).response_mode;
      }
      if ((body as any).conversation_id && !(body as any).conversationId) {
        (body as any).conversationId = (body as any).conversation_id;
      }
      if ((body as any).prior_messages && !(body as any).priorMessages) {
        (body as any).priorMessages = (body as any).prior_messages;
      }
      if ((body as any).auto_generated_name && (body as any).autoGeneratedName === undefined) {
        (body as any).autoGeneratedName = (body as any).auto_generated_name;
      }
      // Normalize inputs fields
      if ((body as any).inputs) {
        const inputs = (body as any).inputs;
        if (inputs.similarityThreshold === undefined && inputs.similarity_threshold !== undefined) {
          inputs.similarityThreshold = String(inputs.similarity_threshold);
        }
        if (inputs.similarityThreshold !== undefined && typeof inputs.similarityThreshold === 'number') {
          inputs.similarityThreshold = String(inputs.similarityThreshold);
        }
        if (inputs.contextCount === undefined && inputs.context_count !== undefined) {
          inputs.contextCount = inputs.context_count;
        }
      }
