...
def _validate_response(response: httpx.Response, test_name):
    ...
    if response.status_code != 200:
        ...
        # Check for NestJS default error format
            ...
            # Map statusCode to status and try to infer code from message
            status_code = data.get("statusCode")
            message = data.get("message", "")
            if isinstance(message, str):
                if "Invalid API key" in message:
                    inferred_code = "unauthorized"
                elif "Unauthorized" in str(data.get("error", "")):
                    inferred_code = "unauthorized"
                else:
                    inferred_code = "invalid_param"  # Default fallback
            else:
                inferred_code = "invalid_param"  # Array messages
            
            is_known_error = any(
                e["status"] == status_code and e["code"] == inferred_code
                for e in errors
            )
        else:
        ...
        if not is_known_error:
            status_val = data.get("status") or data.get("statusCode")
            code_val = data.get("code") or "unknown"
            _process_log(
                (
                    f"[WARNING]: ({test_name}) Error with status=[{status_val}], "
                    f"code=[{code_val}] not found in error list."
                ),
                "warning",
            )
    return data
...
def _validate_response_fields(
    ...
    if is_error_response:
        # For error responses, be more flexible with the format
        if "status" in response_data and "code" in response_data and "message" in response_data:
        ...
        elif "statusCode" in response_data and "message" in response_data:
            # NestJS default error format
            _process_log(f"[OK]: ({test_name}) NestJS error format validated (statusCode: {response_data.get('statusCode')})")
        else:
            _process_log(f"[ERROR]: ({test_name}) Unknown error response format", "error")
    else:
        # For success responses, use the original validation
        required_fields = _get_required_fields(is_blocking_response, is_error_response)
        _validate_fields(response_data, required_fields, test_name)
...
async def test_invalid_api_key():
    ...
    if response_data:
        ...
        # Check for both our custom format and NestJS format
        if response.status_code == 401:
            if (response_data.get("code") == "unauthorized" or 
                response_data.get("statusCode") == 401):
                _process_log("[OK]: test_invalid_api_key PASSED ✅", "ok")
            else:
                _process_log(
                    "[ERROR]: (test_invalid_api_key) Unexpected error format for invalid API key",
                    "error",
                )
        else:
            _process_log(
                "[ERROR]: (test_invalid_api_key) Expected 401 status for invalid API key",
                "error",
            )
        if any(test for test in _error_list if "test_invalid_api_key" in test):
...
async def test_invalid_user_id():
    ...
    if response_data:
        ...
        # Check for validation error (400) since API key is valid but user validation fails
        if response.status_code == 400 and response_data.get("statusCode") == 400:
            _process_log("[OK]: test_invalid_user_id PASSED ✅", "ok")
        else:
            _process_log(
                "[ERROR]: (test_invalid_user_id) Expected 400 validation error for invalid user ID",
                "error",
            )
        if any(test for test in _error_list if "test_invalid_user_id" in test):
...
async def _missing_field_test(test_name, payload):
    ...
    if response_data:
        ...
        # Accept both our custom error format and NestJS validation errors
        if response.status_code == 400:
            if (response_data.get("code") == "invalid_param" or 
                response_data.get("statusCode") == 400):
                _process_log(f"[OK]: {test_name} PASSED ✅", "ok")
            else:
                _process_log(
                    f"[ERROR]: ({test_name}) Unexpected error format for missing field", "error"
                )
        else:
            _process_log(
                f"[ERROR]: ({test_name}) Expected 400 status for missing field", "error"
            )
        if any(test for test in _error_list if test_name in test):
...
async def test_missing_conversation_id():
    ...
    if response_data:
        _validate_response_fields(
            response_data, True, response.status_code not in [200, 201], "test_missing_conversation_id"
        )
